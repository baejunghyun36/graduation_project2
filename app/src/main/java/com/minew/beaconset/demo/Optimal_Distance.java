...
public class Optimal_Distance extends AppCompatActivity {
    ...
    public static int[] arr = new int[35];
    ...
    public Optimal_Distance start() {
        ...
        min_num=10000000;
        item_location[0][0]=section[0][0];
        item_location[0][1]=section[0][1];

        for (int i = 0; i < N-1; i++) {

            for (int j = 0; j < 2; j++) {
                if (j == 0) {
                    item_location[i+1][j] = item_x[i];
                } else {
        ...
        for(int i=1; i<N; i++) {
            ...
            if(now_x>=0&&now_x<=49&&now_y>=0&&now_y<=49) {
                section_A[a_ind][0]=now_x;
                section_A[a_ind][1]=now_y;

//                arr[cnt++] = item_id[i-1];
//                arr[cnt++]=item_id[i];
                section_A[a_ind++][2]=item_id[i-1];
        ...
        for (int q = 0; q< 5;q++) {
            ...
            //a구역
                ...
                for (int i = 1; i < a_ind; i++) {
                    arr[cnt++] = section_A[section_res[i]][2];

                    if(i==(a_ind-1)) {
                        start[0][0]=section_A[section_res[i]][0];
